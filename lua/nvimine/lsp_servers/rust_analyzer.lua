...
local on_attach = require("nvimine.lsp_servers.mapOverridesOnAttach")("rust")
        ...
        vim.system(cmd, { text = true }, function(output)
            if output.code == 0 then
                if output.stdout then
                    local metadata = vim.json.decode(output.stdout)
                    if metadata and metadata.workspace_root then
                        cargo_workspace_root = vim.fs.normalize(metadata.workspace_root)
                    end
                end
